...
- **Phase 0 core design spec**: [`docs/specs/2026-06-16-phase0-core-design.md`](docs/specs/2026-06-16-phase0-core-design.md)
  ...
  first artifact.
- **Project management in Linear**: project "Agent Saddlery", milestone "Phase 0 — Walking skeleton",
  and vertical-slice issues MAR-5…MAR-13 (0.1 echo loop → 0.8 execution seam + persistence → 0.R
  retrospective), dependency-chained.
...
- **LLM providers**: pluggability lives in our own `LLMProvider` seam; native `AnthropicProvider`
  first (`claude-opus-4-8`, prompt caching, adaptive thinking); `OpenAICompatibleProvider` (≈ all OSS)
  and a gateway adapter (LiteLLM/OpenRouter) are later drop-ins. *Supersedes the survey's "LiteLLM in
  the core."*
